...
export function ExamSponsorBrand({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center ${compact ? 'gap-3' : 'gap-4'}`} aria-label="airunote exams, sponsored by Starbucks Store 9">
    ...
    <figure className={compact ? 'w-24 shrink-0' : 'w-full max-w-[220px] shrink-0'}>
      <div className={`relative overflow-hidden rounded-[1.4rem] border border-[#d99a49] bg-[#f7ead4] shadow-[0_18px_45px_rgba(79,45,20,0.2)] ${compact ? 'aspect-[3/4]' : 'aspect-[3/4]'}`}>
        <Image
          ...
          alt="The cats of Starbucks Store 9"
          fill
          priority={!compact}
      ...
      {!compact && <figcaption className="mt-3 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-[#7d5132]">The cats of Store 9</figcaption>}
    </figure>
  );
}
