...
function AttemptDateTime({ value, emptyLabel }: { value: string | null; emptyLabel: string }) {
  ...
  return (
    <time dateTime={value} title={date.toLocaleString()} className="block min-w-36">
      <span className="block font-medium text-slate-800">{date.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</span>
      <span className="mt-1 block whitespace-nowrap text-xs text-slate-500">{date.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit', second: '2-digit', timeZoneName: 'short' })}</span>
    </time>
  );
}
      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        {tab === 'live' && <div className="space-y-6">
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 px-6 py-4"><h2 className="font-semibold text-slate-950">All attempts</h2><p className="mt-1 text-xs text-slate-500">Dates and times are displayed in your device’s current time zone.</p></div>
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">{report.respondents.map((respondent) => <tr key={respondent.id}><td className="px-5 py-4"><div className="font-medium text-slate-900">{respondent.respondentName}</div><div className="mt-1 text-xs text-slate-500">{respondent.respondentIdentifier || respondent.respondentEmail || `Attempt #${respondent.attemptNumber}`}</div></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${respondent.status === 'completed' ? 'bg-emerald-50 text-emerald-700' : respondent.status === 'terminated' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'}`}>{respondent.status.replace('_', ' ')}</span></td><td className="px-5 py-4"><AttemptDateTime value={respondent.startedAt} emptyLabel="Not recorded" /></td><td className="px-5 py-4"><AttemptDateTime value={respondent.completedAt} emptyLabel="Still answering" /></td><td className="px-5 py-4 whitespace-nowrap text-slate-600">{formatDuration(respondent.timeInExamSeconds)}</td><td className="px-5 py-4 whitespace-nowrap font-medium text-slate-900">{respondent.percentage === null ? 'Pending grade' : `${respondent.earnedPoints}/${respondent.possiblePoints} · ${respondent.percentage}%`}</td><td className="px-5 py-4 text-slate-600">{respondent.focusViolationCount}</td><td className="px-5 py-4">{respondent.status !== 'in_progress' && <button type="button" onClick={() => void issueContinueLink(respondent.id)} className="whitespace-nowrap text-sm font-medium text-blue-700 hover:text-blue-800">Copy continue link</button>}</td></tr>)}</tbody>
              </table>
            </div>
          </section>
        </div>}
