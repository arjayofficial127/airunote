...
export default function NewRecordPage() {
  ...
  const loadCollection = async () => {
    try {
      const res = await collectionsApi.list(orgId);
  ...
  const onSubmit = async (data: Record<string, any>) => {
    try {
      ...
      await recordsApi.create(orgId, slug, { data: recordData });
      toast('Record created successfully', 'success');
      router.push(`/orgs/${orgId}/records/${slug}`);
    } catch (error: any) {
      console.error('Failed to create record:', error);
      toast(error.response?.data?.error?.message || 'Failed to create record', 'error');
    }
  };
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Create Record: {collection.name}</h1>
      </div>
